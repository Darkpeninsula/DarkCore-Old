...
{
    CellPair p = DarkCore::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY());
...
bool Map::Add(Player *player)
{
    ...
    CellPair p = DarkCore::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    if (p.x_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP || p.y_coord >= TOTAL_NUMBER_OF_CELLS_PER_MAP)
    {
...
{
    CellPair standing_cell(DarkCore::ComputeCellPair(obj->GetPositionX(), obj->GetPositionY()));
...
Map::PlayerRelocation(Player *player, float x, float y, float z, float orientation)
{
    ...
    CellPair old_val = DarkCore::ComputeCellPair(player->GetPositionX(), player->GetPositionY());
    CellPair new_val = DarkCore::ComputeCellPair(x, y);
...
void Map::MoveAllCreaturesInMoveList()
{
    while (!i_creaturesToMove.empty())
    {
        ...
        // calculate cells
        CellPair new_val = DarkCore::ComputeCellPair(cm.x, cm.y);
        Cell new_cell(new_val);
...
bool Map::CreatureRespawnRelocation(Creature *c)
{
    ...
    CellPair resp_val = DarkCore::ComputeCellPair(resp_x, resp_y);
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    CellPair xy_val = DarkCore::ComputeCellPair(c->GetPositionX(), c->GetPositionY());
    Cell xy_cell(xy_val);
    if (xy_cell != cur_cell)
    {
...
bool Map::ActiveObjectsNearGrid(uint32 x, uint32 y) const
{
    ...
    for (MapRefManager::const_iterator iter = m_mapRefManager.begin(); iter != m_mapRefManager.end(); ++iter)
    {
        ...
        CellPair p = DarkCore::ComputeCellPair(plr->GetPositionX(), plr->GetPositionY());
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridPair p = DarkCore::ComputeGridPair(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridPair p2 = DarkCore::ComputeGridPair(c->GetPositionX(), c->GetPositionY());
