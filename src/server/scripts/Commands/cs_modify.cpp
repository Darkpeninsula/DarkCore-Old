...
public:
    ...
    static bool HandleModifyJusticeCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_JUSTICE_POINTS, amount);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA, handler->GetNameLink(target).c_str(), target->GetCurrency(CURRENCY_TYPE_JUSTICE_POINTS));
    ...
    static bool HandleModifyValorCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->ModifyCurrency(CURRENCY_TYPE_VALOR_POINTS, amount);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_ARENA, handler->GetNameLink(target).c_str(), target->GetCurrency(CURRENCY_TYPE_VALOR_POINTS));
