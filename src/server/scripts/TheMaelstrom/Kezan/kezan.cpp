...
public:
    ...
    {
        npc_fourth_and_goal_targetAI(Creature* pCreature) : ScriptedAI(pCreature) {}
        ...
        {
            Unit *pTarget = NULL;
            pTarget = me->FindNearestPlayer(5.0f, true);
            if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER)
                if(pTarget->ToPlayer()->GetQuestStatus(28414) == QUEST_STATUS_INCOMPLETE)
					pTarget->ToPlayer()->KilledMonsterCredit(37203, 0);
        }
    };
